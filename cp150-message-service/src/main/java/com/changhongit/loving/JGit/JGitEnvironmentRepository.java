...
@ConfigurationProperties("changhong.config.server.git")
public class JGitEnvironmentRepository extends AbstractScmEnvironmentRepository {
    ...
    @Override
    public String findOne(String application, String label) {
        initialize();
        Git git = null;
        try {
            git = createGitClient();
            ...
            TreeWalk treeWalk = TreeWalk.forPath(repository, application, revTree);
            if (treeWalk == null) {
                logger.error("位置更新Schema 文件名错误!");
                return null;
            } else {
    ...
    public boolean fetchRepos() {
        ...
        try {
            git = this.gitFactory.getGitByOpen(getWorkingDirectory());
            tryFetch(git);
    ...
    @Override
    public boolean pullRepos(String label) {
        Git git = null;

        try {
            git = this.gitFactory.getGitByOpen(getWorkingDirectory());
            Repository repository = git.getRepository();
            Ref ref = repository.getRef(label);
